...
interface Box {
	...
	height?: number
	angle?: number
	// Left off entirely for an entity that has no body at all, which is the case the index has to cope with and
	// the collision broadphase does not.
	shape?: number | 'none'
}

// The blocks one entity would arrive at a worker with.  Boxes are 10x10 rectangles unless the test says
// otherwise, so most of them only have to say where they are.
...
describe('spatial-index', () => {
	describe('search', () => {
		...
		it('finds an entity the box only touches', () => {
			// The box ends exactly on the left edge of the 10 wide entity at x 100.
		...
		it('hands the filter the blocks the entity was indexed with', () => {
			...
			// Only the wide one, picked out by reading its own transform back rather than by its id.
		...
		it('searches from the middle out', () => {
			...
			// The far entity's near edge is at 95, so a reach of 50 falls short of it and one of 95 arrives exactly
			// on it.  The reach is per axis: it is the x one that has to cover the gap, however narrow y is.
	...
	describe('findNearest', () => {
		...
		it('measures to the edge of a shape rather than to its middle', () => {
			// The second entity's centre is further away, but it is wide enough that its edge is nearer.
		...
		it('finds nothing past maxDistance', () => {
			// A 10 wide entity centred at 100 has its near edge at 95.
		...
		it('spends maxResults only on entities the filter accepted', () => {
			...
			// The two nearest are rejected, which must not use up the two results that were asked for.
	...
	describe('what goes into the index', () => {
		it('files a circle under the box it covers whichever way it is turned', () => {
			// A rectangle this size turned 45 degrees would reach out to about 14 on each axis; a circle does not.
		...
		it('files a capsule along the way it is facing', () => {
			// 40 from end to end and 10 thick, lying along the x axis: 20 out that way and only 5 the other.
		...
		it('keeps an entity that has no size at all', () => {
			// The collision broadphase drops these - nothing can overlap them - but they still have a position, and
			// a search is a question about position.
